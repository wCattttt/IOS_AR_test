...
- (void)_reloadData{
    ...
    NSString *urlString = [NSString stringWithFormat:@"http://x.feelapp.cc/api/p3/stream/focus?comment=1&page=%ld&page_size=10", _page];
...
//        NSMutableArray *headerImages = @[].mutableCopy;
        // 判断是下拉刷新、下拉加载更多数据
        if(_tag == 200){    // 上拉加载更多数据
            ...
            [datas enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
                if(idx == 0){
                    // 取得头视图的图片
//                    HotHeaderModel *hotHeaderModel = [[HotHeaderModel alloc] initWithDataDic:[obj[@"banner_list"] firstObject]];
//                    _attentionTableView.hotHeaderModel = hotHeaderModel;
                }else{
...
#pragma  mark 创建表视图
- (void)_createView{
    _attentionTableView = [[AttentionTableView alloc] initWithFrame:self.view.bounds style:UITableViewStyleGrouped];
    // 设置tableView起点
    self.edgesForExtendedLayout = UIRectEdgeNone;
    //    _attentionTableView.data = _data;
    _attentionTableView.isHotVC = YES;
