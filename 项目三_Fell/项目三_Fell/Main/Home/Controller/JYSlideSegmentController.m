...
{
  if (!_titleLabel) {
    _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 10, self.contentView.bounds.size.width, self.contentView.bounds.size.height)];
...
- (UIView *)indicator
{
  if (!_indicator) {
    ...
    _indicator.backgroundColor = self.indicatorColor ? : [UIColor blueColor];
  }
  return _indicator;
}
